...
(function($){
	...
	function setProperties(target){
		...
		state.window.draggable({
			handle: '>div.panel-header>div.panel-title',//,window.window-shadow
			...
			onStopDrag: function(e){
				...
				state.options.top = e.data.top>0?e.data.top:0;
				$(target).window('move');
				state.proxy.remove();
				state.proxy = null;
			}
...
(function($){
	...
	function validate(target){
		...
		function doValidate(vtype, vparam){
			...
			if (rule && value){
				var r2=result[2];
				if(rule=='regrule'||result[1]=='regrule') { r2=result[2];}
				else{r2=eval(result[2]);}   
				var param = vparam || opts.validParams || r2;//eval(result[2])
...
//				}, 0);
		...
		if (opts.toolbar) {
			if ($.isArray(opts.toolbar)){
			    ...
			    var tb = $('<div class="datagrid-toolbar"><div></div></div>').prependTo(panel);
			    var tr = tb.find('div');
			    for (var i = 0; i < opts.toolbar.length; i++) {
			        var btn = opts.toolbar[i];
			        if (btn == '-') {
			            $('<div class="eaf-datagrid-toolbar"><div class="datagrid-btn-separator"></div></div>').appendTo(tr);
			        } else {
			            var td = $('<div class="eaf-datagrid-toolbar"></div>').appendTo(tr);
			            var tool = $('<a href="javascript:void(0)"></a>').appendTo(td);
			            tool[0].onclick = eval(btn.handler || function () { });
			            tool.linkbutton($.extend({}, btn, {
			                plain: true
...
//		var bck = opts.finder.getTr(target, '', 'allbody').find('div.datagrid-cell-check input[type=checkbox]');
	...
	function beginEdit(target, index){
		...
		tr.find('div.datagrid-editable').each(function(){
			...
			if (row[field]) {
			    var text;
			    if (row['RES_' + field + '_EAF_NAME']) text = row['RES_' + field + '_EAF_NAME'];
			    if (row['R_RES_' + field.substr(2) + '_EAF_NAME']) text = row['R_RES_' + field.substr(2) + '_EAF_NAME'];
			    ed.actions.setValue(ed.target, row[field], text);
			}
		});
				...
				setValue: function(target, value,text){
					if (isA(target, name)){
						...
						if (opts.multiple){
						    ...
						    if (text) {
						        $(target)[name]('setText', text);               
						        var state = $.data(target[0], 'textbox');
